        ...
        <div class="dashboard-header">
            <div>
                ...
                <p class="page-subtitle">
                    {{ $adminUser->desa->nama_desa ?? '-' }},
                    {{ $adminUser->kecamatan->nama_kecamatan ?? '-' }}
                </p>
            </div>

            {{-- ✅ PROFIL SUB ADMIN DENGAN NAMA & WILAYAH KERJA --}}
            <div class="admin-profile">
                ...
                <div class="admin-info">
                    <span class="admin-name">{{ $adminUser->nama ?? 'Sub Admin' }}</span>
        ...
        <div class="chart-card">
            <div class="card-header">
                ...
                <div class="chart-filters">
                    <form method="GET" class="filter-form-inline" style="display: flex; gap: 8px; align-items: center;">
                        <select name="tahun" class="filter-select-small" onchange="this.form.submit()">
                            @foreach ($tahunOptions as $thn)
                                <option value="{{ $thn }}" {{ $thn == $selectedTahun ? 'selected' : '' }}>
                                    {{ $thn }}</option>
                            @endforeach
                        </select>
                        <select name="bulan" class="filter-select-small" onchange="this.form.submit()">
                        ...
                        <tbody>
                            @forelse($penarikanTerbaru as $penarikan)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $penarikan->status === 'berhasil' ? 'badge-success' : ($penarikan->status === 'ditolak' ? 'badge-danger' : 'badge-warning') }}">
                        ...
                        <tbody>
                            @forelse($penjemputanTerbaru as $penjemputan)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $penjemputan->status === 'disetujui' ? 'badge-success' : ($penjemputan->status === 'ditolak' ? 'badge-danger' : 'badge-warning') }}">
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        suggestedMax: Math.max(...@json($chartData), 10),
                        ticks: {
